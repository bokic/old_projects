...
type
  TBBilansDocForm = class(TForm)
    ...
    procedure KlasaKeyPress(Sender: TObject; var Key: Char);
...
var
  BBilansDocForm: TBBilansDocForm;

implementation
Uses MainUnit, NovoKontoUnit;
{$R *.DFM}

procedure TBBilansDocForm.UpdateData;
Type
   PTempRec=^TempRec;
   TempRec=Record
      Konto:String[6];
...
For Count:=0 to MainForm.KnigoList.Count-1 Do
   ...
   ((Klasa.ItemIndex=10)Or(Kni^.Konto[1]=Chr(Klasa.ItemIndex+Ord('0')))))Then
      Begin
      If (TempList.Count>0) Then
         For C:=0 to TempList.Count-1 Do
            Begin
            TRec:=TempList.Items[C];
            If (Kni^.Konto=TRec^.Konto) Then
               Begin
               Found:=True;
               ...
               Break;
               End;
            If (CompareStr(Kni^.Konto,TRec^.Konto)<0) Then
               Break;
            End;
      If (Found=False) Then
         Begin
...
If (TempList.Count>0) Then
   For Count:=0 To TempList.Count-1 Do
      ...
      End;
Str(SPocDolzi:0:2,TSt);
PocDolziSuma.Caption:=TSt;
Str(SPocPobaruva:0:2,TSt);
...
If (Copy(TSt,1,3)<>Copy(TRec^.Konto,1,3)) Then
   Begin
   TSt2:=TRec^.Konto;
   GetMem(TRec2,SizeOf(TempRec));
   TRec2^.Konto:=Copy(TSt,1,3);
...
If (Count>13)Then
   ...
   Else
   Begin
   PocDolziSuma.Top:=303-((13-Count)*18);
   PocPobaruvaSuma.Top:=303-((13-Count)*18);
   DolziSuma.Top:=303-((13-Count)*18);
   PobaruvaSuma.Top:=303-((13-Count)*18);
   Saldo.Top:=303-((13-Count)*18);
   If (Count=0)Then
      Begin
      PocDolziSuma.Visible:=False;
      PocPobaruvaSuma.Visible:=False;
...
procedure TBBilansDocForm.FormShow(Sender: TObject);
begin
...
procedure TBBilansDocForm.FormClose(Sender: TObject;
...
procedure TBBilansDocForm.StringGridDrawCell(Sender: TObject; ACol,
...
procedure TBBilansDocForm.OdDatumKeyPress(Sender: TObject; var Key: Char);
begin
If (Key=#13) Then DoDatum.SetFocus;
end;

procedure TBBilansDocForm.DoDatumKeyPress(Sender: TObject; var Key: Char);
begin
If (Key=#13) Then Klasa.SetFocus;
end;

procedure TBBilansDocForm.KlasaKeyPress(Sender: TObject; var Key: Char);
begin
If (Key=#13) Then
   Begin
   ReFresh;
...
procedure TBBilansDocForm.StringGridDblClick(Sender: TObject);
var TSt:String;
   C:Integer;
   KPl:PKPlanData;
begin
TSt:=StringGrid.Cells[1,StringGrid.Row];
if(Length(TSt)=3) Then
   Begin
   for c:=0 to MainForm.KPLanList.Count - 1 do
      begin
